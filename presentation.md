...
* Почему Kubernetes стал стандартом для реализации кластеров контейнеризированных рабочих нагрузок?
* Что было до и в основе Kubernetes?
* Введение в концепцию и разнообразие объектов Kubernetes
* Немного устройства и абстракций
* Практическая демонстрация Hello World приложения
...
Провиженинг ОС, доставка, настройка и запуск ПО на ВМ ничем не отличается от этих процедур на железе

* Провиженинг ОС: конфиги, переменные окружения, данные - всё общее, всё может пересечься в рамках одной ВМ
* Доставка ПО: пакеты, архивы, архивы сорцов с необходимостью сборки
* Для разного ПО зависимости могут конфликтовать
...
Провиженинг ОС, доставка, настройка и запуск ПО проходят совершенно по-другому

* Провиженинг: необходимо и достаточно установить и настроить Container Runtime
* Доставка: использование готовых **образов**
...
* Запуск: необходимо и достаточно установить и настроить Container Runtime

---

![bg_contain](./images/docker.jpg)
...
* В образе только необходимые рантайм зависимости для запуска целевого ПО
* Запуск на любой ОС с подходящим рантаймом
...
* Катастрофически не хватает абстракций окружения: ингрессы, сервисы, ConfigMaps / Secrets, PVC, StorageClass...
